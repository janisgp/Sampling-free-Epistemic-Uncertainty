...
parser.add_argument('--gpu',                  type=str,   help='GPU to use for training', default='0')
...
parser.add_argument('--use_dropout',                 type=int,   help='use dropout? 0: no ; 1: yes!', default=0)

args = parser.parse_args()

# set dropout boolean
args.use_dropout = bool(args.use_dropout)

# define run folder...
args.log_directory = 'runs/' + args.model_name + str(round(time.time()))[:-2] + '/'
...
#                 model = MonodepthModel(params, args.mode, left_splits[i], right_splits[i], reuse_variables, i)
                model = MonodepthModel(params, args.mode, left_splits[0], right_splits[0], reuse_variables, 0, use_dropout=args.use_dropout)
...
def test(params):
    ...
    model = MonodepthModel(params, args.mode, left, right, use_dropout=args.use_dropout)
